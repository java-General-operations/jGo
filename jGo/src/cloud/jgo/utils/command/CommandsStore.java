 ...
 * 
 * @author Martire91
 * This class Represents a store of predefined commands that can be executed and ready to use.
 * And from time to time will be updated with new useful commands..
 * 
 */
public class CommandsStore implements Serializable{
	...
	public static final LocalCommand ALERT_COMMAND = new RemoteCommand("alert", "This command opens the custom alert"); // ha un oggetto condiviso
	...
	private static Parameter createParam,identifiesParam,renameParam,deleteParam,
	copyParam,moveParam,writeParam,readParam,openParam,getParam;
	...
	static{
		...
		/**
		 * @author Martire91
		 * init params command - file
		 * createParam,renameParam,deleteParam,copyParam,moveParam,writeParam;
		 * create : prende un valore da input cioè il path del file e crea un oggetto condiviso che è il file stesso
		 * 
		 */
		...
		getParam = FILE_COMMAND.addParam("get","This parameter returns the file"); // in realtà lo passa a un metodo che noi effettuandone l'override siamo in grado di lavorare sul file
		...
		createParam.setExecution(new Execution() {
			...
			public Object exec() {
				...
				// restituisco la risposta che in questo caso è una stringa che comunica l'esito della creazione in questo caso
		...
		renameParam.setExecution(new Execution() {
			...
			public Object exec() {
				...
				if(FILE_COMMAND.getSharedObject()!=null){
					...
					£.moveFile(((File)FILE_COMMAND.getSharedObject()),new File(((File)FILE_COMMAND.getSharedObject()).getParentFile(),newName));
        ...
        typeParam.setExecution(new Execution() {
         ...
         showParam.setExecution(new Execution() {
			...
			public Object exec() {
				...
				JOptionPane.showMessageDialog(null, joptionPane.getText(),joptionPane.getTitle(),joptionPane.messageType,joptionPane.getIcon());
