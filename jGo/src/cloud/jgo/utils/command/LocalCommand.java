...
public class LocalCommand implements Command,Iterable<Entry<String, Parameter>>,Shareable{
	...
	public boolean hasInputHelpExploitable(){
	 ...
	 public static class HelpCommand implements Serializable,Comparable<HelpCommand>{
		...
		public void reload(LocalCommand command) {
			...
			buffer.append("=================================================================================================================================\n");
			buffer.append("HELP Of "+"\""+this.command.command+"\" - Phase :"+((LocalCommand)this.command).getBelongsTo()+"\n");
			buffer.append("=================================================================================================================================\n");
			...
			if(this.command.hasParameters()){
				...
				while (iterator.hasNext()) {
					...
					buffer.append(param.getParam()+"="+param.getParameterHelp()+"  / has input value ="+param.hasInputValueExploitable()+"\n");
				}
			}
		}
	...
	/**
	 * Questo metodo va spiegato bene :
	 * lavora con la stampa di sistema
	 * mette a disposizione una struttura
	 * di get and set per le variabili degli oggetti condivisi su cui si lavora.
	 * per settare per esempio la var nome di persona mi basterà fare :
	 * 
	 ...
	 * 
	 * persona -nome
	 * Questo discorso, attenzione è solo per le variabili degli oggetti condivisi
	 * poichè il metodo quando riceve un comando che vuole accedere a una var,recupera
	 * l'oggetto condiviso di quel comando e su di esso tenta di recuperare quella variabile,
	 * il valore della variabile viene stampato anche nel caso in cui sia null, appunto 
	 * per avvisare l'utente di questo valore nullo
	 *
	 */
	// a questo metodo importantissimo ho tolto lo static
	// quindi casomai da problems vedere un attimino o aggiungere lo static al metodo
				...
				if(rest.equals(LocalCommand.helpValue)){
				   ...
				   if(getCommand.hasInputHelpExploitable()){
					...
					if(entries.size()>0){
						...
						if(getCommand!=null){
							/***
							 * @author MARTIRE91 : QUI è IMPORTANTE
							 * SE UN GIORNO VOGLIAMO ESEGUIRE UN COMANDO CHE
							 * HA UNA ESECUZIONE E ANCHE DEI PARAMS E QUI
							 * CHE DOBBIAMO ESEGUIRLO,PERO C'è SOLO UN PROBLEMA
							 * POI E' DA TENERE PRESENTE CHE SE PER ESEMPIO DIAMO 
							 * UN COMANDO SIMILE : person -toString, lui reinizializza
							 * di nuovo la persona e quindi ci stampa valori nulli
							 * per il momento, l'esecuzione del comando ha valore
							 *  solo se quest'ultimo non ha params,cosa che prima o poi dovremo correggere.
							 */
							...
							for (int i = 0; i < entries.size(); i++) {
								...
								if(getCommand.isParameter(entry.param)){
									...
									// ottengo il param vero e proprio per poi controllare se ha un valore da input sfruttabile
...
//														System.out.println(objectReturn);	
											...
											if(entry.value!=null){
											    objectReturn = "The parameter ("+getParameter+") has not a exploitable input value #";
//												System.out.println(objectReturn);
											    commandReturnList.add(objectReturn);
											    objectReturn = commandReturnList;
											}
	...
	public <T> void shareObject(T sharedObject) {
	    ...
	    if (this.sharedObject!=null) {
	    	...
	    	if (!isParameter("to_string")) {
	    	    ...
	    	    final Parameter pToString = 
	    	    this.addParam("to_string","This parameter shows the toString method of the shared object");
