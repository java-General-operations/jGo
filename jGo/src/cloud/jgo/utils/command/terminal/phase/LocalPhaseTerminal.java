...
public class LocalPhaseTerminal extends LocalTerminal implements Structure{
	 ...
	 * @param phaseDescription the phase description
	...
	public Phase createPhase(final int value,String phaseName,String phaseDescription,Command...commands){
		...
		if (phase(value)==null && phase(phaseName)==null) {
			...
			// questa lo commentata perchè è più giusto che i comandi che si aggiungono ad una fase non si aggiungano anche ai comandi del terminale
			if (commands!=null) {
			...
			// qui solo se non è la fase start, perchè non ci serve avere un riferimento a tale fase
			// eseguiamo il codice dell'aggiungimento link param. Tutto questo perchè abbiamo il metodo reset
			...
			if (value>1) { // diamo per scontato che la prima fase è la numero 1
				...
				Parameter p_link = pointerCommand.addParam(phase.phaseName(),"Go to ("+phase.phaseName()+") phase @");
			...
			// anche se la fase è la start mi serve attribuire una descrizione 
						Parameter p_link_desc = describerCommand.addParam(phase.phaseName(),"This parameter describes the ("+phase.phaseName()+") phase @");
	...
	public final void implOpen() {
		...
		if(command!=null){
			...
			if (command.hasAPhase()) {
				...
				/* Non ha la fase
				 * qui eseguo il comando come se fosse
				 * nel localTerminal
				 */
			...
			public Object exec() {
				...
				if (currentPhase!=null) {
					...
					for (int i = 0; i <commands.size(); i++) {
						...
						buffer.append((i+1)+")"+commands.get(i).getCommand()+"="+commands.get(i).getHelp()+"\n");
					}
	...
	// current phase. Quest'ultima viene modificata solo all'interno di questo metodo
	// eppoi nella sottoclasse del terminale, ma il cambiamento del suo valore e a noi impercettibile
	/**
	...
	public Phase nextPhase() {
		// quindi qui in tanto andiamo ad individuare il valore numerico della fase corrente
