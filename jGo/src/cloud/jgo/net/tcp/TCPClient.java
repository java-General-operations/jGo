 ...
 * and is a subclass of {@link Client}
 *
 */
 ...
 * @param commands the client commands
 */
 ...
 * spiegazione comandi :
 * 
 * * clientCommands : sono i comandi impostati dal client per controllare il server
 * * serverCommands : sono i comandi impostati dal server per controllare il client
 * 
 */

/**
 * Questo metodo controlla i comandi che riceve il client dal server ,come infatti
 * si nota nel codice che si va a cercare nell'array di comandi impostati dal server.
 * Ora se trova il comando lo restituisce ma se il comando ha dei params
 * a quel punto,il metodo cerca di restituire kmq il comando padre dei comandi con params.
 * Per esempio se io ho un comando registrato tipo cmd -s che serve ad aprire il cmd
 * ,se io passo questo comando al metodo ,lui non mi ritorna direttamente il comando
 * cmd -t,ma mi restituisce il comando padre di quest'ultimio quindi il comando cmd.
 * Poi c'è un altra cosa da sapere su questo metodo,diciamo che questo metodo è un metodo predisponente
 * a un altro metodo che è il metodo statico della classe RemoteCommand :ExecuteInputRemoteCommandWithReturn
 * oppure ExecuteInputRemoteCommand.
 * Ultima informaizone non meno importante :
 * Se per esempio nel client abbiamo registrato dei soli comandi senza params allora nel lato server side
 * si può benissimo utilizzare solo il metodo checkReceivedCommand che retutn il comando
 * e a quel punto basta eseguire il comando.Incese se nel client abbiamo registrato una gerarchia di comandi
 * ,quindi ci sono comandi con params a quel punto nel lato server side ,basta invocare prima il metodo
 * checkReceivedCommand e dopo il metodo statico : ExecuteInputRemoteCommandWithReturn
 * oppure ExecuteInputRemoteCommand.Tutto questo è quello che c'è da sapere.
 * @param command
 * @return
 */
...
public RemoteCommand checkCommand(String command, List<RemoteCommand> commands) {
     ...
     * @throws IOException 1 exception
     * @throws ClassNotFoundException 2 exception
     */
	 ...
	 * This method uses a Proxy.
	 * @param host the remote host
	 * @param port the remote port
	 * @param proxy the proxy (SOCKS)
	 * @return true is connected
	 * @throws IOException 1 exception
	 */
	public boolean connectWithProxy(String host,int port,Proxy proxy) throws IOException { // qui bisogna che sia stato impostato il proxy
		if(proxy!=null){
			try {
