...
public abstract class TCPServer implements Server, Manageable, Iterable<Handler> {
	 ...
	 *            the executable
	 * @param valueNameRegistry
	 *            the register value that you want to associate
	...
	/**
	 * This method is useful when extending this class. Set the model type you will
	 * be working on at a later time.
	 * 
	 * @param handler
	 *            the model type you want to use
	 */
	...
	public boolean isConfigurated() {
		...
		if (this.getConfiguration() != null) {
			...
			// ora qui devo controllare se il numero delle impostazioni settate è maggiore
			// di 0
	 ...
	 * 
	 * @param configuration
	 *            the configuration with which the server is started
	...
	public String toString() {
		/*
		 * 
		 * JGO Auto-generated method stub Author : £ wasp91 £ Date 23 nov 2017
		 * 
		 */
		...
		if (this.nameServer != null) {
			...
			try {
				...
				/*
				 * 
				 * JGO Auto-generated catch block Author : £ wasp91 £ Date 23 nov 2017
				 * 
				 */
				return null;
			}
	 ...
	 *             1 exception
	 * @throws NoReadingSourceException
	 *             2 exception :the read source has not been set
	 */
	public DefaultSocket acceptRequest() throws IOException, NoReadingSourceException {
		...
		if (this.getConfiguration().containsKey(TCPServerConfiguration.MAXIMUM_SOCKETS)) {
			...
			if (this.countSockets < ((Integer) this.configuration2.getConfig(TCPServerConfiguration.MAXIMUM_SOCKETS))) {
				...
				if (this.getConfiguration().containsKey(TCPServerConfiguration.DEFAULT_PRINT_FOR_ACCEPTANCE_SOCKET)) {
				...
				try {
					...
					/*
					 * 
					 * JGO Auto-generated catch block Author : £ wasp91 £ Date 05 feb 2018
					 * 
					 */
					e.printStackTrace();
				}
	...
	protected void acceptRequestsFromClientsWithModel() throws IOException, CloneNotSupportedException,
		...
		while (true) {
			...
			if (this.acceptedConnection != null) {
				...
				/*
				 * JGO NON SO SE DA PROBLEMI EVENTUALMENTE TOGLIERE TUTTO QUESTO CODICE
				 * DELL'ELSE
				 */
				/*
				...
				// e qui chiudiamo il metodo usando return ,tanto ormai sono gli handlers
				// threads a pensare a gestire le connessioni
	...
	protected void acceptRequestWithModel()
			throws IOException, CloneNotSupportedException, HeadlessException, ClassNotFoundException, AWTException,
			InstantiationException, IllegalAccessException, NoReadingSourceException {
		this.acceptedConnection = acceptRequest();
		Class classeModel = model.getClass();
		TCPHandlerConnection newInstance = (TCPHandlerConnection) classeModel.newInstance();
		...
		try {
			...
			/*
			 * 
			 * JGO Auto-generated catch block Author : £ wasp91 £ Date 22 nov 2017
			 * 
			 */
			e.printStackTrace();
		}
	...
	@Override
	public void listen(int localPort, String positiveListen, String negativeListen)
			throws NoReadingSourceException, NegativeListeningException {
		this.localPort = localPort;
		...
		if (occupied) {
			...
			try {
				...
				read(positiveListen);
				this.inListen = true;
				if ((Integer) this.getConfiguration().getConfig(TCPServerConfiguration.TIMER) != null) {
	...
	public void listen() throws NegativeListeningException {
		...
		if (occupied) {
			...
			if ((Integer) getConfiguration().getConfig(TCPServerConfiguration.TIMER) != null) {
