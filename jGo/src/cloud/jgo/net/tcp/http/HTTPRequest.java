...
public class HTTPRequest implements HTTPMessage{
     ...
     * @param decoder_type the http decoder type
     */
	...
	public HTTPRequest(String infoRequest,String rootFolder, int serverPort, boolean notifyConnections) throws UnknownHostException, UnsupportedEncodingException, MalformedURLException, MimeTypeParseException {
		...
		for (int i = 0; i < split.length; i++) {
			if(notifyConnections==true){
				 ...
				 * invece noi dobbiamo stampare nella sorgente impostata dall'utente,però
				 * questo lo posso risolvere con calma
				 */
		...
		//JOptionPane.showMessageDialog(null,this.serverAddress,"host server",JOptionPane.INFORMATION_MESSAGE);
		...
		/*LOCATION
		 * la locazione del file nel server (Molto utile)
		 */
		...
		/*
		 * OKOKO eliminare da qui a @ in caso di problemi
		...
		if(rowCacheControl!=null){
			if (!rowCacheControl.isEmpty()) {
				...
				if(splitForTwoPoint.length==2){
					...
					cacheControl = splitForTwoPoint[1].trim(); // cancello lo spazio iniziale che rimane dopo i due punti
		...
		// verifico se la versione http contiene appunto la dicitura
			...
			/**
			 * @author Martire91
			 * E entrato nell'else quindi split e inferiore di due
			 */
	 ...
	 * @return decoded url
	 * @throws UnsupportedEncodingException 1 exception
	 * @throws MalformedURLException 2 exception
	 */
