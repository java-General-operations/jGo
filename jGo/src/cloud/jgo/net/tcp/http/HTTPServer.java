...
/**
 * @author Martire91<br>
 *         This class represents a simple http server, and is a subclass of
 *         {@link TCPServer}
 */
public class HTTPServer extends TCPServer {
	...
	protected void acceptRequestsFromClientsWithModel() throws IOException, CloneNotSupportedException,
		...
		while (loopInfinity) {
			...
			this.acceptedConnection = this.acceptRequest(); // qui diciamo che se la socket restituita è diversa da null
															// si agisce
			if (this.acceptedConnection != null) {
				...
				if (this instanceof HTTPServer) {
					if (((HTTPServer) this).getRootFolder() != null) {
						...
						try {
							((cloud.jgo.net.tcp.http.HTTPHandlerConnection) newInstance)
									.setSocket(this.acceptedConnection, this.rootFolder);
	...
	public boolean isConfigurated() {
		...
		if (tcpConfigurated == false) {
			...
			if (this.configuration2.containsKey(HTTPServerConfiguration.ROOT_FOLDER.getKey())) {
				if (new File((String) this.configuration2.getConfig(HTTPServerConfiguration.ROOT_FOLDER)).exists()
						&& new File((String) this.configuration2.getConfig(HTTPServerConfiguration.ROOT_FOLDER))
								.isDirectory()) {
					return true;
				} else {
					return false;
	...
	public void impl(Handler handler) {
		// TODO Auto-generated method stub
		System.out.println(
				"Connection from " + ((HTTPHandlerConnection) handler).getSocket().getRemoteSocketAddress().toString());
