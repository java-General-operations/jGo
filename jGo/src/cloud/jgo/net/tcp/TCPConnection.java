...
public class TCPConnection implements Connection,Controllable,Control{
	...
	public boolean enterRemoteCommand(RemoteCommand cmd,Parameter...params){
		...
		if (cmd!=null) {
			...
			// quindi qui non devo fare altro che mandare un simpleEntry al client con il comando e l'array di params
    				...
    				if (param.hasInputValueExploitable()) {
    					...
    					if (inputValue!=null) {
    						...
    						if (((RemoteCommand)param.getParent()).getSharedObject()!=null) {
    							...
    							objects.add("The parameter ("+param.getOnlyParam()+") requires a value from input #");
    						}
    					...
    					if(inputValue!=null){
    						...
    						objects.add("The parameter ("+param.getOnlyParam()+") has not a exploitable input value #");
	...
	public TCPConnection(Socket socket,boolean successfully,TCPClient client) throws IOException, ClassNotFoundException {
		...
		if(this.successfully){
			...
			List<cloud.jgo.utils.command.RemoteCommand>serverCommands = (List<cloud.jgo.utils.command.RemoteCommand>) this.in.readObject();
	 ...
	 * differenze :
	 * 
	 * 1 differenza :  Questo metodo non restituisce i valori get impostati precedentemente su un oggetto
	 * 2 differenza :  Non restituisce il metodo toString 
	 * 3 restituisce un array di oggetti che vanno poi distinti nel lato opposto a cui si inviano
	 * 4 non invia comunicazioni se tipo non abbiamo dato il value input e il param lo richiedesse
	 * e non a comunicazioni se tipo mettiamo un value input e il param non lo  richiedesse
	 */
	@Override
	public ArrayList<Object> executeRemoteCommand() {
		...
		if(entry!=null){
			...
			// qui verifico se stiamo parlando come secondo param del simple entry di un array di params stringhe o proprio params
				...
				if (params!=null) {
					if (params.length>0) {
						...
						for (int i = 0; i < params.length; i++) {
							...
							// allora qui controllo se il param in questione non ha un valore da input sfruttabile e allora lo eseguo
